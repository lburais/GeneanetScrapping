...
class Individual(Element):
    ...
    @property
    def source(self):
        """Return the value of the Source URL (_GEN) of this person, or None if no Source URL."""
        try:
    ...
    def set_source(self, url):
        """
        Set the Source URL for this person.

        :param str url: Source URL.
        :raises TypeError: if `url` is invalid
        """
        if url == None:
            raise TypeError("Must have source URL value")
        try:
            src_node = self['_SRC']
            src_node.value = url
        except IndexError:
            self.add_child_element(self.gedcom_file.element("_SRC", level=1, value=url))

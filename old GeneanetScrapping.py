...
class GPerson(GBase):
    ...
    def __init__(self,level):
        ...
        display(_("Person::__init__ - Initialize Person at level %d")%(level), level=2, verbose=1 )
    ...
    def scrap_geneanet(self, purl):
        ...
        display(_("Person::scrap_geneanet: %s")%(purl), level=2, verbose=1 )

        import nodriver as uc

        async def read_geneanet( purl ):

            try:
                # Start the browser
                browser = await uc.start(headless=False)
            
                page = await browser.get( purl)
        ...
        tree = uc.loop().run_until_complete(read_geneanet(purl))
        #tree = read_geneanet(purl)
...
# get_geneanet
# -------------------------------------------------------------------------
# retrieve one person from geneanet
